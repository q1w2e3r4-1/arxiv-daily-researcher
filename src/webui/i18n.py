...
_TRANSLATIONS: dict[str, dict[str, str]] = {
    ...
    "scoring_hint": {
        "zh": "可选择传统关键词加权评分，或先用低成本模型初筛、再按需触发 SMART_LLM 复核的 MLSys 评分。",
        "en": "Choose either legacy keyword-weighted scoring or the staged MLSys scorer that screens with cheap models and optionally escalates to SMART_LLM.",
    },
    ...
    "committee_hint": {
        "zh": "先由这些低成本模型顺序初筛；若初筛均分落入边界区间，再额外调用一次 SMART_LLM。重复失败时按 fallback 分数计入最终平均。",
        "en": "These cheap models score first; if their average lands in the borderline range, one SMART_LLM review is added. Repeated failures still contribute the fallback score to the final average.",
    },
    "committee_models_label": {"zh": "初筛模型（每行一个）", "en": "First-pass models (one per line)"},
    "committee_models_help": {
        "zh": "这些模型走 CHEAP_LLM 接口，按顺序完成第一阶段初筛。",
        "en": "These models use the CHEAP_LLM endpoint and run sequentially for the first-pass screen.",
    },
    ...
    "committee_smart_review_title": {"zh": "SMART_LLM 复核", "en": "SMART_LLM review"},
    "committee_smart_review_enabled": {"zh": "启用边界复核", "en": "Enable borderline review"},
    "committee_smart_review_min": {"zh": "复核下界", "en": "Review lower bound"},
    "committee_smart_review_max": {"zh": "复核上界", "en": "Review upper bound"},
    "committee_smart_review_help": {
        "zh": "当初筛均分落入该区间时，额外调用一次 SMART_LLM，并将其分数并入最终平均。",
        "en": "If the first-pass average lands in this range, one SMART_LLM judgment is added to the final average.",
    },
