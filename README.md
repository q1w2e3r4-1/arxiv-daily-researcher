...
6. **针对Eurosys 2026的评测结果进行分析**：我们对之前跑的那轮评测结果进行了详细分析，发现了模型在准确率、召回率、F1分数等方面的表现，并且比较了单模型和委员会的结果。总体来说，glm-5.1表现最好，但委员会在暴露可能的标签误差方面也很有价值。虽然这个结果对项目本身没啥用，而且ground truth也不是很truth, 不过依然为未来可能的测试提供了一个基准。

7. **Docker 定时任务遇锁时会等待重试**：为了避免每天的自动任务恰好撞上本机手动开发/测试运行，Docker 的 cron daily run 在遇到 `daily_research` 锁时，会每 30 分钟重试一次，最多 40 次（共 20 小时）。这个等待逻辑只对 Docker 自动任务生效；手动 CLI、开发测试和 WebUI 手动运行仍然保持“撞锁即快速跳过”的原有行为。
