...
2. **使用大模型作为智能筛选工具**：不同于原版主要依赖关键词权重和简单规则进行筛选，我们引入了 `CHEAP_LLM`（如 qwen3.5-27b）来对每篇论文的标题和摘要进行逐关键词评分，并生成综合评分与理由。这使得筛选过程更智能，能够捕捉到一些隐式相关性，而不仅仅是显式关键词匹配。

3. **使用分阶段多模型筛选文章**：在 `mlsys_multi_model` 策略下，系统会先让多个低成本模型对论文标题与摘要做第一阶段筛选，并计算初筛均分，超过6分算接受；只有当均分落入边界区间（5-7分）时，才额外调用一次 `SMART_LLM` 复核。最终是否通过，取决于最终平均分是否达到阈值；若某个模型连续重试后仍失败，则记为带错误标记的 fallback 结果，并按 5 分计入平均，避免单个模型异常变成隐式反对票。

4. **使用“致远一号”模型作为服务提供商**: 这东西真是量大管饱，一个月10B token, 目前想不到怎么花，唯一的缺点就是速度慢以及模型不是很新，但对于这种非实时任务来说完全够用。当前运行时的推荐拆分是：`glm-5.1` 作为 `SMART_LLM` 负责边界复核，`minimax-m2.7`、`qwen3.5-27b`、`deepseek-v3.2` 作为第一阶段低成本初筛模型。我们之前也对这四个模型做过统一评测，结果显示它们整体表现都不错，甚至有些指标上比之前用的模型还要好。

5. **以单个论文为单位进行处理和持久化**: 与原版一次性处理所有论文不同，我们改为以单篇论文为单位进行评分、分析、结果持久化(SQLite)等操作。这样做的好处是可以更细粒度地追踪每篇论文的处理状态，避免因为某篇论文处理失败而影响整个批次的结果。同时也方便我们在后续分析中对每篇论文的评分和分析结果进行更深入的挖掘(例如按研究者或者日期筛选)。

6. **新增数据库汇总界面**：接着上一点，我们对数据库中的所有论文数据进行汇总分析，方便用于按照照研究者、日期、关键词等维度进行统计和分析。这种方式可以解决原版中只能按照批次进行分析的局限性（基本都是每日一个报告，但是你又不能把多日的报告合并，或者本身做一些筛选什么的）

7. **针对Eurosys 2026的评测结果进行分析**：我们对之前跑的那轮评测结果进行了详细分析，发现了模型在准确率、召回率、F1分数等方面的表现，并且比较了单模型和委员会的结果。总体来说，glm-5.1表现最好，但委员会在暴露可能的标签误差方面也很有价值。虽然这个结果对项目本身没啥用，而且ground truth也不是很truth, 不过依然为未来可能的测试提供了一个基准。

8. **Docker 定时任务遇锁时会等待重试**：为了避免每天的自动任务恰好撞上本机手动开发/测试运行，Docker 的 cron daily run 在遇到 `daily_research` 锁时，会每 30 分钟重试一次，最多 40 次（共 20 小时）。这个等待逻辑只对 Docker 自动任务生效；手动 CLI、开发测试和 WebUI 手动运行仍然保持“撞锁即快速跳过”的原有行为。

运行docker compose up -d --build， 就会自动构建并启动服务，每天1点自动开始更新(如果你更新了源码，也需要运行这个命令来同步)。
---
