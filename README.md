...
2. **使用分阶段多模型筛选文章**：在 `mlsys_multi_model` 策略下，系统会先让多个低成本模型对论文标题与摘要做第一阶段筛选，并计算初筛均分，超过6分算接受；只有当均分落入边界区间（5-7分）时，才额外调用一次 `SMART_LLM` 复核。最终是否通过，取决于最终平均分是否达到阈值；若某个模型连续重试后仍失败，则记为带错误标记的 fallback 结果，并按 5 分计入平均，避免单个模型异常变成隐式反对票。

3. **使用“致远一号”模型作为服务提供商**: 这东西真是量大管饱，一个月10B token, 目前想不到怎么花，唯一的缺点就是速度慢以及模型不是很新，但对于这种非实时任务来说完全够用。当前运行时的推荐拆分是：`glm-5.1` 作为 `SMART_LLM` 负责边界复核，`minimax-m2.7`、`qwen3.5-27b`、`deepseek-v3.2` 作为第一阶段低成本初筛模型。我们之前也对这四个模型做过统一评测，结果显示它们整体表现都不错，甚至有些指标上比之前用的模型还要好。
